...
void *Compare_EPS(void* rank) {
	...
	while(1) {
		...
		int EPS_CURRENT_RANGE = abs(EPS_BATT[EPS_CURRENT_VAL]-CURRENT_SAFE);
		int EPS_VOLTAGE_RANGE = abs(EPS_BATT[EPS_VOLTAGE_VAL]-VOLTAGE_SAFE);
		int EPS_TEMPERATURE_RANGE = abs(EPS_BATT[EPS_TEMPERATURE_VAL]-TEMPERATURE_SAFE);
		
		// update CURRENT state 
		if( CURRENT_SAFE-WARNING_THRESHOLD <= EPS_BATT[EPS_CURRENT_VAL] <= CURRENT_SAFE+WARNING_THRESHOLD ) { 
			EPS_BATT[EPS_CURRENT_STATE] = NOMINAL;
			
		} else if( CURRENT_SAFE-DANGER_THRESHOLD <= EPS_BATT[EPS_CURRENT_VAL] <= CURRENT_SAFE+DANGER_THRESHOLD  ) {
		...
		if( VOLTAGE_SAFE-WARNING_THRESHOLD <= EPS_BATT[EPS_VOLTAGE_VAL] <= VOLTAGE_SAFE+WARNING_THRESHOLD ) { 
			EPS_BATT[EPS_VOLTAGE_STATE] = NOMINAL;
			
		} else if( VOLTAGE_SAFE-DANGER_THRESHOLD <= EPS_BATT[EPS_VOLTAGE_VAL] <= VOLTAGE_SAFE+DANGER_THRESHOLD  ) {
		...
		if( TEMPERATURE_SAFE-WARNING_THRESHOLD <= EPS_BATT[EPS_TEMPERATURE_VAL] <= TEMPERATURE_SAFE+WARNING_THRESHOLD ) { 
			EPS_BATT[EPS_TEMPERATURE_STATE] = NOMINAL;
			
		} else if( TEMPERATURE_SAFE-DANGER_THRESHOLD <= EPS_BATT[EPS_TEMPERATURE_VAL] <= TEMPERATURE_SAFE+DANGER_THRESHOLD  ) {
